; This script is used to build the Windows install program for Rocq.
...
; VERSION      Rocq version, e.g. 8.5-pl2
...
!define MY_PRODUCT "Rocq-Platform${VERSION}" ;Define your own software name here
!define OUTFILE "Rocq-Platform-release-${RELEASE}-version${VERSION}-Windows-${ARCH}.exe"
...
Section "-CoqWinStuff" Sec_CoqWinStuff
  ...
  ${registerExtension} "$INSTDIR\bin\${IDE_NAME}.exe" ".v" "Rocq Script File"
  ...
  WriteRegStr HKEY_LOCAL_MACHINE "Software\Microsoft\Windows\CurrentVersion\Uninstall\${MY_PRODUCT}" \
      "Publisher" "The Rocq Team"
  WriteRegDWORD HKEY_LOCAL_MACHINE "Software\Microsoft\Windows\CurrentVersion\Uninstall\${MY_PRODUCT}" \
      "NoModify" "1"
  WriteRegDWORD HKEY_LOCAL_MACHINE "Software\Microsoft\Windows\CurrentVersion\Uninstall\${MY_PRODUCT}" \
      "NoRepair" "1"
  WriteRegStr HKEY_LOCAL_MACHINE "Software\Microsoft\Windows\CurrentVersion\Uninstall\${MY_PRODUCT}" \
      "URLInfoAbout" "https://rocq-prover.org/"

  ; Create start menu entries
  ; SetOutPath is required for the path in the .lnk files
  SetOutPath "$INSTDIR"
  CreateDirectory "$SMPROGRAMS\Rocq"
  ; The first shortcut set here is treated as main application by Windows 7/8.
  ; Use Rocqide as main application
  CreateShortCut "$SMPROGRAMS\Rocq\${IDE_NAME}.lnk" "$INSTDIR\bin\${IDE_NAME}.exe" "" "$INSTDIR\bin\Rocq.ico" 0
  ; Rocq shell
  CreateShortCut "$SMPROGRAMS\Rocq\Rocq-Shell.lnk" "%COMSPEC%" "/K $INSTDIR\rocq-shell.bat" "$INSTDIR\rocq-shell.ico" 0
  ; CreateShortCut "$SMPROGRAMS\Rocq\Rocq.lnk" "$INSTDIR\bin\coqtop.exe"
  WriteINIStr "$SMPROGRAMS\Rocq\The Rocq HomePage.url" "InternetShortcut" "URL" "https://rocq-prover.org/"
  WriteINIStr "$SMPROGRAMS\Rocq\The Rocq Standard Library.url" "InternetShortcut" "URL" "https://coq.inria.fr/distrib/current/stdlib/"
  CreateShortCut "$SMPROGRAMS\Rocq\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
...
; The section index variables are only defined after the section definitions.
  ...
  !define MUI_DIRECTORYPAGE_TEXT_TOP "ATTENTION:$\r$\n -the path must not include spaces$\r$\n- the path must be empty unless you are adding additional packages using the same installer$\r$\n- if you already have a *different* Rocq version installed in the *same* place, please uninstall first via add/remove programs$\r$\n- it is fine to install multiple versions of Rocq in *different* folders"
...
Section "Uninstall"
  ...
  Delete "$INSTDIR\rocq-shell.ico"
  Delete "$INSTDIR\rocq-shell.bat"

  ; Start Menu
  Delete "$SMPROGRAMS\Rocq\${IDE_NAME}.lnk"
  Delete "$SMPROGRAMS\Rocq\rocq-shell.lnk"
  Delete "$SMPROGRAMS\Rocq\Uninstall.lnk"
  Delete "$SMPROGRAMS\Rocq\The Rocq HomePage.url"
  Delete "$SMPROGRAMS\Rocq\The Rocq Standard Library.url"
  Delete "$INSTDIR\Uninstall.exe"

  ; Registry keys
  DeleteRegKey HKCU "Software\${MY_PRODUCT}"
  DeleteRegKey HKLM "SOFTWARE\Rocq"
  DeleteRegKey HKLM "SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\${MY_PRODUCT}"
  ${unregisterExtension} ".v" "Rocq Script File"

  ; Root folders
  RMDir "$INSTDIR"
  RMDir "$SMPROGRAMS\Rocq"
